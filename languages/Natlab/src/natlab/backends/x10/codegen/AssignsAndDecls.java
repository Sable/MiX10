...
public class AssignsAndDecls {
	...
	public static void handleTIRAbstractAssignToVarStmt(
		...
		if (true == target.symbolMap.containsKey(target.symbolMapKey)
				&& (target.parforSwitch == false || isReductionStmt == true)) {
			isDecl = false;
			...
			if (null != assign_stmt.getLHS().getShape())
				for (int i = 0; i < assign_stmt.getLHS().getShape().size(); i++) {
					if (null != assign_stmt.getLHS().getShape().get(i))
						tf &= ("1").equals(assign_stmt.getLHS().getShape()
								.get(i).toString());
				}
			...
			decl_stmt.getLHS().setName(
					((TIRAbstractAssignToVarStmt) node).getLHS().getVarName());
	...
	public static void setRHSValue(boolean isDecl, Stmt decl_or_assgn,
			TIRAbstractAssignStmt node, boolean isScalar,
			IRx10ASTGenerator target) {

		if (isDecl) {

			((DeclStmt) decl_or_assgn).setRHS(Expressions.makeIRx10Exp(
					node.getRHS(), isScalar, target));
		} else {
			((AssignStmt) decl_or_assgn).setRHS(Expressions.makeIRx10Exp(
					node.getRHS(), isScalar, target));
		}
	...
	public static void handleTIRAbstractAssignToListStmt(
		...
		if (1 == ((TIRAbstractAssignToListStmt) node).getTargets().asNameList()
			...
			if (true == target.symbolMap.containsKey(target.symbolMapKey)
					&& (IRx10ASTGenerator.parforSwitch == false || isReductionStmt == true)) {
		...
		// TODO: Fix this to be handled in parfor
			...
			for (ast.Name name : ((TIRAbstractAssignToListStmt) node)
				...
				if (!target.symbolMap.containsKey(name.getID())) {
					...
					partDeclStmt.setLHS(Helper.generateIDInfo(target.analysis,
			...
			for (int i = 0; i < list_assign_stmt.getMultiAssignLHS()
					.getNumIDInfo(); i++) {
				pseudoAssign = new AssignStmt();
				accessMultiRet = new ArrayAccess();
				pseudoAssign.setLHS(list_assign_stmt.getMultiAssignLHS()
						.getIDInfo(i));
				accessMultiRet.setArrayID(new IDUse(multiVarName));
				accessMultiRet.getIndicess().add(
						new IntLiteral(Integer.toString(i)));
				pseudoAssign.setRHS(accessMultiRet);
				pseudoAssign.setTypeCast(true);
				block.addStmt(pseudoAssign);
				target.symbolMap.put(pseudoAssign.getLHS().getName(),
						pseudoAssign.getLHS());
			}
		}
